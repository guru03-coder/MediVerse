...
export const MockDB = {
    getStats: () => {
        // Recalculate queue stats dynamically
        const high_risk_waiting = patients.filter(p => p.risk_level === 'CRITICAL' || p.risk_level === 'High').length;
    ...
    addDoctor: (deptName: string, _docName: string) => {
        // In a real DB we'd add row to Doctors table
        // Here we just increment the active count for the demo effect
        // match deptName to key
        if (departments[deptName as keyof typeof departments]) {
            departments[deptName as keyof typeof departments].active_doctors += 1;
            return true;
        }
        return false;
    },
